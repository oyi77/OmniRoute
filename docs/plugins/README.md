...
- `onRequest(ctx)`: Fires before an LLM request is sent. Return `{ body: { ... } }` to modify the request before it reaches the provider.
...
> **Note:** `onModelSelect` and `onComboResolve` are defined as built-in events but are not yet wired into the routing pipeline. Use `onRequest` to intercept and rewrite `body.model` instead.
...
- `"file-read"` / `"file-write"`: Gives access to a sandboxed `fs` object scoped _only_ to your plugin's directory.
...
**Goal:** Learn how to build an intelligent cost-saving router using `onRequest`.
This plugin hooks into `onRequest` to count the approximate token length of the user prompt. If the prompt exceeds the configured threshold, it rewrites `body.model` to a cheaper fallback model (e.g. `gpt-3.5-turbo`) before the request reaches the executor.
...
_Security Note:_ The `ctx` object passed to hooks is deeply cloned via IPC serialization. You cannot access `fs`, `process.env`, or `child_process` natively unless explicitly granted via the Plugin Manifest `permissions` block.
