...
export default definePlugin({
  ...
  // Additional hooks: onInstall, onActivate, onDeactivate, onUninstall
  // (see src/lib/plugins/hooks.ts for the full BUILTIN_EVENTS list)
};
```

For the complete list of supported built-in events and their signatures, refer to `src/lib/plugins/hooks.ts` (`BUILTIN_EVENTS` constant). Routing and stream events (`onModelSelect`, `onComboResolve`, `onRateLimit`, etc.) are planned but not yet available — use `onRequest`/`onResponse` for request-level interception.
...
> **⚠️ Not yet wired up.** `startDevMode()` is exported but has **no call sites** (`grep -rn "startDevMode" src/ open-sse/` returns only the definition). Neither `OMNIROUTE_PLUGIN_DEV` nor any other env var turns dev mode on — the `PLUGIN_DEV_MODE` string appears only as the **pino logger name** in `src/lib/plugins/devMode.ts:12`. Dev mode is therefore only invokable programmatically today; an env-var/CLI path that turns it on is planned but not implemented.
...
- **Errors during reload**: the old version remains active. Check the server log (the `devMode` and `pluginManager` loggers) for the error.
...
Dev mode has a 500ms debounce. Wait a moment, then check the server log for `devMode` events.
