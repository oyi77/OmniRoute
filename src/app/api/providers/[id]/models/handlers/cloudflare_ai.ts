...
export async function handleCloudflare_aiModels(ctx: ModelsRequestContext): Promise<any> {
  ...
  try {
    ...
    if (!res.ok) {
      return buildDiscoveryErrorFallbackResponse(
        ...
        `Cloudflare Workers AI returned status ${res.status}`
      );
    }
    ...
    return buildDiscoveryErrorFallbackResponse(err, "Failed to fetch Cloudflare Workers AI models");
  }
}
