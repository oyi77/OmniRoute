...
import { markAccountUnavailable } from "./accountManagement.ts";
import { CredentialSelectionOptions } from "./types.ts";
...
export const selectionMutexes = new Map<string, Promise<void>>();

export function getSelectionMutexKey(provider: string, options: CredentialSelectionOptions): string {
  return [
...
export function createSelectionLock(key: string) {
...
export const markMutexes = new Map<string, Promise<void>>();